    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float CalculateEdge(float2 uv, float2 delta)
            {
                    ...
                    float3 s[9];
                    s[0] = GetSample(uv + float2(-delta.x, -delta.y)); s[1] = GetSample(uv + float2(0, -delta.y)); s[2] = GetSample(uv + float2(delta.x, -delta.y));
                    s[3] = GetSample(uv + float2(-delta.x, 0));        s[4] = GetSample(uv);                       s[5] = GetSample(uv + float2(delta.x, 0));
                    s[6] = GetSample(uv + float2(-delta.x, delta.y));  s[7] = GetSample(uv + float2(0, delta.y));  s[8] = GetSample(uv + float2(delta.x, delta.y));
                    float3 gx = s[2] + s[8] + 2*s[5] - s[0] - s[6] - 2*s[3];
                    float3 gy = s[6] + s[8] + 2*s[7] - s[0] - s[2] - 2*s[1];
                    float3 sq = gx*gx + gy*gy;
                #else
                    float3 s1 = GetSample(uv + float2(-delta.x, -delta.y)); float3 s2 = GetSample(uv + float2(delta.x, delta.y));
                    float3 s3 = GetSample(uv + float2(-delta.x, delta.y));  float3 s4 = GetSample(uv + float2(delta.x, -delta.y));
                    float3 d1 = s1 - s2; float3 d2 = s3 - s4;
            ...
            float CalculateSelectionEdge(float2 uv, float2 delta)
            {
                float s0 = SAMPLE_TEXTURE2D(_SelectionMaskTexture, sampler_LinearClamp, uv).r;
                ...
                float diff = abs(s0 - s1) + abs(s0 - s2) + abs(s0 - s3) + abs(s0 - s4);
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                float edge = 0;
                    ...
                    float selEdge = CalculateSelectionEdge(uv, delta);
